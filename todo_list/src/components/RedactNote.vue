<template>
    ...
    <div class="navbar-start">
      <div class="tooltip tooltip-right" data-tip="Отменить изменения">
        <button class="btn btn-ghost text-4xl btn-circle save" @click="$router.push('/')">
          <!--<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M3 12L20 12M3 12l5 5M8 7l-5 5" /></svg>-->
  ...
  <div class="delete">
    <button class="btn btn-lg btn-circle" @click="showPopup">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 6L20 6M6 6l0 15M17 6l0 15M6 21L17 21M8 2L15 2" /></svg>
    </button>
  </div>
...
const note = noteStore.notes.find((note) => note.id === Number(noteId));

const count = note.original_note.length;
const formCount = ref(count);

newNoteTitle.value = note.original_title;
newNote.value = note.original_note.slice();
newDone.value = note.is_done.slice();
...
const handleSubmit = () => {
      note.original_title = newNoteTitle.value;
      note.original_note = newNote.value;
      note.is_done = newDone.value;
